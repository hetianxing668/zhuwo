...
final class SlightPHP{
	...
	/**
	 * 断点调试
	 * @param object $obj 要输出的调试对象
	 * @param string $doExit 是否终止
	 * @param string $doFormat 是否格式化
	 * @return object 返回调试信息
	 */
	public static function breakPoint(&$obj,$doExit=true,$doFormat=true){
		$type = gettype($obj);
		if($doFormat){
			echo '<pre>';
		}
